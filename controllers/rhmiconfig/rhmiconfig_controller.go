...
func (r *RHMIConfigReconciler) Reconcile(request ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.reconcileValues(rhmiConfig, reconcileBackupAndMaintenanceValues, reconcileUpgradeValues); err != nil {
		log.Error(err, "failed to reconcile rhmi config values")
		return retryRequeue, err
	}

	// create cloud resource operator override config map
	if err := r.ReconcileCloudResourceStrategies(log, rhmiConfig); err != nil {
		log.Error(err, "rhmi config failure while reconciling cloud resource strategies")
		return retryRequeue, err
	}
...
func (r *RHMIConfigReconciler) ReconcileCloudResourceStrategies(log logr.Logger, config *rhmiconfigv1alpha1.RHMIConfig) error {
